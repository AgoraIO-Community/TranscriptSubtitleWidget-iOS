...
class MessageView: UIView {
    ...
    private func makeAttributedText(item: RenderInfo) -> NSAttributedString {
        ...
        var translateRenderInfos = item.translateRenderInfos
        translateRenderInfos.sort { (info1, info2) -> Bool in
            return info1.lang < info2.lang
        }
        for (index, translateRenderInfo) in translateRenderInfos.enumerated() {
    ...
    func getAllTranslateText(insertFinalTag: Bool = false) -> String {
        var textDict = [String : [String]]()
        for item in items {
            for translateRenderInfo in item.translateRenderInfos {
                if !translateRenderInfo.text.isEmpty {
                    if textDict[translateRenderInfo.lang] == nil {
                        textDict[translateRenderInfo.lang] = [String]()
                    }
                    textDict[translateRenderInfo.lang]!.append(translateRenderInfo.text)
                }
            }
        }
        
        var texts = [String]()
        textDict.keys.sorted().forEach { lang in
            let text = textDict[lang]!.joined()
            texts.append(text)
        }
    ...
    @objc public func getAllTranscriptText(insertFinalTag: Bool = false) -> String {
        var texts = [String]()
        for item in items {
            var text = ""
            text += item.transcriptText
            let transcriptFinalCharacter = insertFinalTag ? ( (item.transcriptRanges.last?.isFinal ?? false) ? "[F]" : "[NF]" ) : ""
            text += transcriptFinalCharacter
            texts.append(text)
        }
        return texts.joined(separator: "\n\n")
    }
